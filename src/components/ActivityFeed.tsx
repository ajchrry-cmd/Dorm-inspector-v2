  ...
  const getActivityText = (activity: typeof activities[0]) => {
    switch (activity.type) {
      case 'started_inspection':
        return (
          <>
            <span className="font-medium">{activity.inspectorName}</span> started inspecting{' '}
      ...
      case 'completed_inspection':
        return (
          <>
            <span className="font-medium">{activity.inspectorName}</span> completed{' '}
            <span className="font-medium">Room {activity.roomNumber}</span>
            {activity.result && (
              <span
                className={`ml-1 px-1.5 py-0.5 rounded text-xs font-medium ${
                  activity.result === 'outstanding'
